...
import {skipAuth} from "@/utils/auth";
import {isMockMode, initMockMode} from "@/sandbox";


// 创建axios实例
const axios = Axios.create({
    baseURL: isMockMode ? '' : process.env.VUE_APP_BaseURL, // api 的 base_url
            ...
            switch (result.code) {
                case 401:
                    if (skipAuth) {
                        Notification({
                            title: '提示',
                            message: result.message || '未登录或会话已失效（开发模式未跳转登录页）',
                            type: 'warning',
                            showClose: true,
                            duration: 5 * 1000
                        });
                    } else if (logoutNoticeSemaphore > 0) {
