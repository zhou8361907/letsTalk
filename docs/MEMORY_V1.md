...
├──────────────────────────────────────────────────────────────────┤
│ E0   Episodic · session_search + episodic_recall（见 SESSION_SEARCH_V1）│
├──────────────────────────────────────────────────────────────────┤
...
| **Tier 2** | 每轮 user prefix | INDEX → `<memory_context>`；历史意图 → `<episodic_recall>`（FTS 自动） | 已落地 |
| **Tier 3** | Agent 按需工具 | memory · save/read_memory · session_search | 已落地 |
...
- **background_review**：默认每 10 user turn（`LETS_TALK_MEMORY_NUDGE_INTERVAL=0` 可关闭）。
...
| 文件 | 路径 | 硬顶 | 谁写 |
|------|------|------|------|
| USER | `.agent/memory/USER.md` | 1500 字符 | 人 + Agent（`memory` target=user，或 `update_user_profile`） |
| CORE | `.agent/memory/CORE.md` | 2500 字符 | 人 + Agent（`memory` target=core，或 `update_core_memory`） |
...
| session_search | Phase C → 见 [STATE_DB_V1.md](./STATE_DB_V1.md) |
...
### Phase C（Phase A+B 已完成，此阶段部分已落地）

7. background_review 每 N 轮可写 0~1 条
8. ✅ **session_search** 已落地（state.db FTS；见 [STATE_DB_V1.md](./STATE_DB_V1.md) 及 [SESSION_SEARCH_V1.md](./SESSION_SEARCH_V1.md)）
...
| [STATE_DB_V1.md](./STATE_DB_V1.md) | 会话冷存储 state.db（Phase C / E0） |
