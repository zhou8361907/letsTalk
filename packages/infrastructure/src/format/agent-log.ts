import type { AgentStepLogFields, LogStep, RequestLogContext } from "@lets-talk/infrastructure/logging";

const STEP_LABELS: Record<LogStep, string> = {
  "route.bundle_load": "加载模块",
...
export function formatPrettyLogLine(
  ...
  const redundantDetail = detail === STEP_LABELS[fields.step];
  const detailSuffix =
    detail && !redundantDetail ? c(ANSI.dim, ` · ${detail}`) : "";
